...
class RobotArmEnvV0(gym.Env):
    ...
    def generate_random_angle(self):
        ...
        for i in range(self.n_links):
            theta[i] = random.uniform(self.min_theta, self.max_theta)
        return theta
...
class RobotArmEnvV2(RobotArmEnvV0):
    '''any num of links'''
    def __init__(self, num_links=4, link_length=60):
        super(RobotArmEnvV2, self).__init__()
        self.nLinks = num_links
        self.min_action = -1.0
        self.max_action = 1.0
        self.min_theta = np.radians(0)
        self.max_theta = np.radians(90)
    ...
    def step(self, action):
        for i in range(self.nLinks):
            self.theta[i] = np.interp(action[i], (self.min_action, self.max_action), (self.min_theta, self.max_theta))
...
class RobotArmEnvV3(RobotArmEnvV0):
    '''any num of links'''
    def __init__(self, num_links=4, link_length=60, max_episode_steps=200):
        super(RobotArmEnvV3, self).__init__()
        ...
        self.action_space = spaces.Box(low=self.min_action, high=self.max_action, shape=(self.nLinks,), dtype=np.float32)
        self.set_link_properties([link_length]*self.nLinks)
        obs_dim = len(self.theta) + 2 + 1 + 1 #2: tip_pos, 1: distance_error, 1: float(done)
        self.observation_space = spaces.Box(np.finfo(np.float32).min, np.finfo(np.float32).max, shape=(obs_dim,), dtype=np.float32) 
    ...
    def step(self, action):
        ...
        observation = np.hstack((self.theta, np.array(tip_pos)/100, np.array([distance_error]), np.array([float(done)])))
        
        max_step = 0
        if self._step_cnt == self._max_episode_steps:
            done = True
            max_step = 1
    ...
    def reset(self):
        ...
        tip_pos = [P[-1][0,3], P[-1][1,3]]
        distance_error = euclidean(self.target_pos, tip_pos)
        observation = np.hstack((self.theta, np.array(tip_pos)/100, np.array([distance_error]), np.array([0])))
        return observation
